"""Packaged Palmer email triage workflow pilot.

This lives under ``hermes_workflows.examples`` deliberately: it is a real-ish
dogfood workflow that should be importable from an installed package, but it is
still fixture-only pilot code. It writes local proposal artifacts only after a
human approval gate and never sends, archives, deletes, marks, drafts, mutates
calendar/account state, changes credentials, or touches live cron jobs.
"""
...
PILOT_APPROVAL_KEY = "approve_email_triage_packet"
REGISTRY_NAME = "palmer-email-triage-pilot"
WORKFLOW_REF = "hermes_workflows.examples.email_triage:palmer_email_triage_workflow"
...
DANGEROUS_SIDE_EFFECT_KEYS = (
    ...
    "emails_deleted",
    "emails_archived",
    "read_unread_mutations",
    "gmail_drafts_created",
    "gmail_drafts_deleted",
    "calendar_mutations",
    "account_actions",
    "payments_or_purchases",
    "oauth_or_auth_changes",
    "external_http_requests",
)

SAFE_LOCAL_SIDE_EFFECT_KEYS = (
    "local_artifacts_written",
    "kanban_tasks_created",
    "kanban_comments_added",
    "skyvault_files_written",
)

_POLICY = {
    "pilot_mode": True,
    "fixture_only_by_default": True,
    ...
    "forbidden": [
        ...
        "delete Gmail draft",
        "calendar mutation",
        "account/security/payment/credential action",
        "cron replacement or mutation",
    ],
}
...
def empty_side_effect_ledger() -> dict[str, int]:
    ledger = {key: 0 for key in DANGEROUS_SIDE_EFFECT_KEYS}
    ledger.update({key: 0 for key in SAFE_LOCAL_SIDE_EFFECT_KEYS})
    return ledger
...
async def fetch_email_triage_candidates(ctx: Any, inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    if fixture == "synthetic":
        ...
        source = {
            ...
            "fixture": "palmer-email-triage-v1",
        ...
        raise ValueError("email triage pilot only accepts fixture='synthetic' or fixture='provided'")
    ...
    return {
        ...
        "account_health": {
            ...
            "note": "Pilot fixture run; no Gmail reads or auth changes performed.",
        },
    }
...
async def render_email_triage_approval_packet(ctx: Any, classified_packet: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "title": "Palmer email execution triage workflow pilot packet",
        "summary": _counts_summary(counts),
        "approval_key": PILOT_APPROVAL_KEY,
        ...
        "dangerous_side_effects_zero": True,
        "raw_private_email_included": False,
...
@step
async def perform_email_triage_pilot_writebacks(
    ctx: Any,
    ...
    receipt_packet = {
        ...
        "db_alias": inputs.get("db_alias") or "email-triage-pilot",
...
@workflow
async def palmer_email_triage_workflow(ctx: Any, inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    decision = await ctx.approval.request(
        (
            "Approve Palmer email triage pilot local writebacks? "
    ...
    return await perform_email_triage_pilot_writebacks(ctx, packet, decision, inputs)
...
def sanitize_email_triage_inputs(inputs: Any, *, workflow_id: str | None = None) -> dict[str, Any]:
    if not isinstance(inputs, dict):
        raise ValueError("email triage pilot inputs must be a mapping")
    ...
    sanitized: dict[str, Any] = {"fixture": fixture, "approver": "human:skylar"}
...
def _local_writeback_paths(output_dir: str | Path) -> dict[str, str]:
    ...
    return {
        "triage_packet": str(root / "triage-packet.json"),
        "kanban_proposal": str(root / "kanban-proposal.md"),
        "skyvault_proposal": str(root / "skyvault-proposal.md"),
        "side_effect_ledger": str(root / "side-effect-ledger.json"),
    }
...
def _default_output_root() -> str:
    return f"dist/email-triage-workflow-pilot-{date.today().isoformat()}"
...
def _classification_reason(classification: str) -> str:
    return {
        "draft_reply": "Candidate appears to need a response; pilot proposes a local draft outline only.",
        "kanban_update": "Candidate contains active-work context; pilot proposes a local Kanban comment/task note only.",
        "archive_candidate": "Candidate appears low-attention, but pilot records only the candidate and performs no archive.",
        "ignore": "Candidate does not need attention; pilot records suppression only.",
        "human_decision": "Candidate needs Skylar judgment; pilot records the ask without external action.",
        "auth_blocked": "Candidate cannot be processed safely without auth/context; pilot records blocker only.",
    }[classification]
...
def _kanban_proposal(receipt: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Local Kanban proposal — email triage workflow pilot",
            "",
            ...
            "",
            "No Kanban rows were mutated by this pilot run.",
            "Proposed writebacks:",
            *[
...
def _skyvault_proposal(receipt: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Local Skyvault proposal — email triage workflow pilot",
            "",
            "No Skyvault files were changed by this pilot run.",
            f"Workflow: {receipt['workflow_id']}",
