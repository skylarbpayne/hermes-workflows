...
def test_gather_enqueues_all_missing_steps_before_waiting(tmp_path):
    ...
    engine.start(fanout_workflow, {"left": 1, "right": 2}, workflow_id="wf_gather")
    result = engine.worker_once("wf_gather", worker_id="worker-start")

    assert result.status == "waiting"
    assert result.waiting_on == "gather:0"
    assert RUNS == []
    commands = engine.pending_commands("wf_gather")
    assert [command["key"] for command in commands if command["type"] == "run_step"] == ["step:gather_left:0", "step:gather_right:0"]
...
def test_gather_resumes_after_each_worker_child_completes(tmp_path):
    ...
    engine.start(fanout_workflow, {"left": 1, "right": 2}, workflow_id="wf_gather")
    first = engine.worker_once("wf_gather", worker_id="worker-start")
    ...
    assert after_left.status == "running"
    after_left = engine.worker_once("wf_gather", worker_id="worker-left-resume")
    ...
    assert after_right.status == "running"
    after_right = engine.worker_once("wf_gather", worker_id="worker-right-resume")
    ...
    events = engine.events("wf_gather")
    assert [(event["type"], event["key"]) for event in events[:5]] == [
        ("WorkflowStarted", "workflow:start"),
        ("CommandClaimed", "workflow:run"),
...
def test_gather_results_follow_call_order_when_children_complete_out_of_order(tmp_path):
    ...
    assert after_right.status == "running"
    after_right = engine.worker_once("wf_gather", worker_id="worker-after-right")
    ...
    assert after_left.status == "running"
    after_left = engine.worker_once("wf_gather", worker_id="worker-after-left")
